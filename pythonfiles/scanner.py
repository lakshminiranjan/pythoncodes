...
#Define your target
if len(sys.argv) == 2:#argv is like $(n) where here we are assigning the n as 2(max only two arg are allowed)
    target = socket.gethostbyname(sys.argv[1])#Translating a hostname to ipv4(With the help of this we can even give the hostnames(hostname or ip dd shouls be in $!)
else:
...
#Exceptions-getting the open port throw exceptions in python.
try:#do our operations at here.
...
else:#same like finally block in java
    print("The total ports scanned are :"+ str(port))
